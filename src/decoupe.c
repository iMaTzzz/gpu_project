                        ...
                        struct bitstream *stream, uint8_t h1, uint8_t v1, uint8_t h2, uint8_t v2, uint8_t h3, uint8_t v3)
{
    ...
    // TODO => Faire 3 différents tableaux pour chaque composante ou créer un seul tableau pour toute la ligne
    // qui contient toutes les composantes
    int16_t *mcus_line_array_Y = malloc(height_mcu * mcus_line_array_width * sizeof(int16_t));
    if (mcus_line_array_Y == NULL) {
        printf("malloc for mcus line array Y failed\n");
        exit(EXIT_FAILURE);
    }
    int16_t *mcus_line_array_Cb = malloc(height_mcu * mcus_line_array_width * sizeof(int16_t));
    if (mcus_line_array_Cb == NULL) {
        printf("malloc for mcus line array Cb failed\n");
        exit(EXIT_FAILURE);
    }
    int16_t *mcus_line_array_Cr = malloc(height_mcu * mcus_line_array_width * sizeof(int16_t));
    if (mcus_line_array_Cr == NULL) {
        printf("malloc for mcus line array Cr failed\n");
        exit(EXIT_FAILURE);
    }
    uint16_t *index = malloc(sizeof(uint16_t));
    ...
    /* On alloue tous les espaces mémoire nécessaires. */
    // On utilise une MCU différente pour chaque composante.  
    uint8_t **mcu_Y = alloc_mcu(width_mcu, height_mcu); 
        ...
        if (tronc_down && mcu_line == nb_mcu_column - 1) {
            ...
            for (uint8_t line = 0; line < height_remainder; ++line) {
                ...
                for (column = 0; column < width; ++column) {
                    ...
                    mcus_line_array_Y[(column / 8) * 64 + line * 8 + column % 8] = fgetc(image);
                }
                // Troncature à droite possible que sur la dernière colonne de MCU
                if (tronc_right) {
                    ...
                    for (uint8_t column_offset = column_index; column_offset < width_mcu; ++column_offset) {
                        ...
                        // On copie la valeur précédente pour remplir le reste de la ligne
                        uint32_t row_in_last_mcu = (nb_mcu_line - 1) * 64 + line * 8;
                        mcus_line_array_Y[row_in_last_mcu + column_offset] = mcus_line_array_Y[row_in_last_mcu + column_offset - 1];
                    }
                }
            ...
            for (uint8_t line_offset = height_remainder; line_offset < 8; ++line_offset) {
                ...
                for (column = 0; column < width; ++column) {
                    // TODO => Modifier accès en fonction de la taille des MCUs et rajouter Cb et Cr
                    uint32_t same_column_in_same_mcu = (column / 8) * 64 + column % 8;
                    mcus_line_array_Y[same_column_in_same_mcu + line_offset * 8] = mcus_line_array_Y[same_column_in_same_mcu + (height_remainder - 1) * 8];
                }
                // Troncature à droite possible que sur la dernière colonne de MCU
                if (tronc_right) {
                    uint8_t column_index = column % width_mcu;
                    for (uint8_t column_offset = column_index; column_offset < width_mcu; ++column_offset) {
                        // TODO => Modifier accès en fonction de la taille des MCUs et rajouter Cb et Cr
                        // On copie la valeur précédente pour remplir le reste de la ligne
                        mcus_line_array_Y[(nb_mcu_line - 1) * 64 + line_offset * 8 + column_offset] = mcus_line_array_Y[(nb_mcu_line - 1) * 64 + (height_remainder - 1) * 8 + column_index - 1];
                    }
                }
            }
        } else { // Pas de troncature vers le bas ou on ne se trouve pas sur la dernière ligne de MCU
            for (uint8_t line = 0; line < height_mcu; ++line) {
                ...
                if (tronc_right) {
                    ...
                    for (uint8_t column_offset = column_index; column_offset < width_mcu; ++column_offset) {
                        ...
                        // mcus_line_array[line * mcus_line_array_width + column_offset] = mcus_line_array[line * mcus_line_array_width + column_offset - 1];
                        uint32_t row_in_last_mcu = (nb_mcu_line - 1) * 64 + line * 8;
                        mcus_line_array_Y[row_in_last_mcu + column_offset] = mcus_line_array_Y[row_in_last_mcu + column_index - 1];
                    }
                }
            }
        }
